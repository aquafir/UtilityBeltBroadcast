...
{
    public class ExClientBase : IDisposable
    {
        ...
        public ExClientBase(int clientId, string connectionId, Action<string> log, Action<Action> runOnMainThread, SerializationBinder binder)
        {
            ClientId = clientId;
                ...
                ))
                {
                    ...
                    if (!isReadingMessageBody)
                    {
                        ...
                        using (var reader = new BinaryReader(stream))
                        {
                            ...
                            //Logger.Debug($"Received message with header {incomingMessageHeader.Type.ToString()}");
